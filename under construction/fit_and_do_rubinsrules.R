...
fit_and_do_rubinrules<-function(dff_imputed, methods_list, pop_params_z, 
    ...
    for (m in 1:M_pva){
      ...
      out_ftc_m = fit_til_convergence(p = p , z = z, temp_wd_p_vec = temp_wd_p_vec, 
                                      ...
                                      readModels = TRUE, 
                                      Model_txt = out_ftc_complete$model_txt, savedata = FALSE, output_txt = "tech3;")
      ...
      hi = out_ftc_m$out_readModels$tech3$paramCov
      V_m = tril(hi) + t(tril(hi,-1))
      params_switch_m = params_switch_m %>% arrange(tech1_to)
      V_m_switched = V_m[params_switch_m$tech1_from, params_switch_m$tech1_from]
      
      list_params[[m]] = params_switch_m
      
      list_V_m[[m]] = as.matrix(V_m_switched) #Q-by-Q-by_M_pva with within imputation asympototic covariance
      list_theta_m[[m]] = params_switch_m$est #Q-by-M_pva with parameter column vectors
      
    } # end for m 
    
    
    V_m_array = array(dim = c(nrow(V_m), ncol(V_m), M_pva)) + NA
    theta_m_mat = mat.or.vec(nr = nrow(V_m), nc = M_pva)
    for (m in 1:M_pva){
      V_m_array[,,m] = as.matrix(list_V_m[[m]])
      theta_m_mat[,m] = as.numeric(list_theta_m[[m]])
    }
    ...
    for (m in 1:M_pva){
      dt_m = as.numeric(theta_m_mat[,m])-t_bar
    ...
    out_list = list(parameters = parameters_df, 
                    ARIV = ARIV,
                    Vtot = Vtot,
                    Vbar = Vbar,
                    B = B, 
