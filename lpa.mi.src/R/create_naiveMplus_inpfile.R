...
#' @export
#' @examples
#' create_naiveMplus_inpfile(z, data_conditions, dffolderfiles, temp_wd_p_vec)
create_naiveMplus_inpfile<-function(out_get_FMM, dffolderfiles, temp_wd_p_vec, type_imputation = FALSE, starts_txt = "0;", symbol_txt = "*"){
  ...
  K_z = out_get_FMM$K_z
  full_mu_z = out_get_FMM$mu_z
  full_S_z = out_get_FMM$S_z
  pi_z = as.numeric(out_get_FMM$pi_z)
...
# symbol_txt = "*";
  ...
  temp_wd_p = temp_wd_p_vec[1]
    ...
    #### Model Command ####
        ...
        marginals_txt = paste("[c#1*", round(log(pi_z[1]/pi_z[K_z]), 5), "];", sep = "")
        if(K_z>2){
          for(k in seq(2,K_z-1)){
            marginals_txt = c(marginals_txt, 
                             paste("[c#",k,symbol_txt, round(log(pi_z[k]/pi_z[K_z]), 5), "];", sep = ""))
          }
        }
        ...
        for (k in c(1:K_z)){
          withinclass_txt = naive_withinclass_Mplus_syntax(mu_k = round(full_mu_z[1:J_Y_z,k],5), 
                                                     S_k = round(full_S_z[1:J_Y_z,1:J_Y_z, k],5),
                                                     usev_vec = usev_vec,
    ...
    #### Make and save the input files ####
        ...
        for(h in seq(1,Htot)){
          ...
          fileConn<-file(paste("Naive ", sub(pattern = ".dat", ".inp",x = dffolderfiles_z$files[h]), sep = ""))
