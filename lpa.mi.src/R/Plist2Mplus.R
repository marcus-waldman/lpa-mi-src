#' Convert population Plist to Mplus format
#'
#' @param z (integer)
#' @param data_conditions (data.frame)
#'
#' @return (data.frame) with population parameter values
#' @export
#'
#' @examples
Plist2Mplus<-function(z, data_conditions){
    ...
    Plist = get_Plist(z = z, data_conditions = data_conditions)
    ...
    means_df = with(FMM_params,
                    ...
                    )) %>% transform(value = as.vector(Plist$mu))
    ...
    var_df = with(FMM_params,
                  data.frame(paramHeader = rep("Variances", K_z*J_Y_z),
                             param = rep(paste0("Y",1:J_Y_z),K_z),
                             LatentClass = as.character(sort(rep(1:K_z,J_Y_z)))
                  )) %>% transform(value =  as.vector(apply(Plist$S,
                                                            MARGIN = 3,
                                                            FUN = function(x){x[diag(x)]})) )
    ...
    cov_df = with(FMM_params,
                  ...
                  ))%>% transform(value = as.vector(apply(Plist$S,MARGIN = 3,
                                                          FUN = function(x){x[lower.tri(x, diag = FALSE)]}
                                                          ))) %>%
                  arrange(LatentClass)

    pivec = as.numeric(Plist$pi)
