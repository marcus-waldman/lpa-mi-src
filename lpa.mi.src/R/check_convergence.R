...
check_convergence<-function(file, folder_wd = getwd(), starts_txt = "0;", type_imputation = FALSE){
...
#file = "Naive dfcom p1 z1 rep15 v2.out"
    ...
    if(type_imputation == FALSE){
        ...
        if (length(i_0)>0){
          if (starts_txt=="0;"){Converge = TRUE}
          if (starts_txt!="0;" & length(i_1)==1){Converge = TRUE}
        }
    }
