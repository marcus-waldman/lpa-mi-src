...
#df = list_get_complete$dfcom
  ...
  for(pm in seq(1,length(pctmiss_vec))){
    ...
    if (save_it == TRUE){
      ...
      save(obsdf_z,
           file = paste(temp_wd_p,"/Observed data/obsdf p", p, " z", z," rep", rep, " pm", pm,".RData",sep = ""))
      
      # Save a ".dat" file for Mplus
      prepareMplusData(obsdf_z, keepCols = 1:J,
                       filename = paste(temp_wd_p,"/Observed data/obsdf p", p, " z", z, " rep", rep, " pm", pm,".dat",sep = ""), inpfile = FALSE, 
                       overwrite = TRUE)
    }
